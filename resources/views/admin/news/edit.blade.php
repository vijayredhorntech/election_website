    ...
    <nav>
        <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
            ...
            <li class="text-sm pl-2 capitalize leading-normal text-black before:float-left before:pr-2 before:text-black before:content-['/']"
                aria-current="page">Edit News
            </li>
        </ol>
        <h6 class="mb-0 font-bold text-black capitalize">Edit News</h6>
    </nav>
    @endsection
                    ...
                    <div class="border-black/12.5 mb-0 rounded-t-2xl border-b-0 border-solid p-6">
                        <form action="{{ route('admin.news.update', $news->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-6">
                                ...
                                <input type="text" id="title" name="title" value="{{ old('title', $news->title) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                                @error('title')
                            ...
                            <div class="mb-6">
                                ...
                                <textarea id="content" name="content" rows="8" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>{{ old('content', $news->content) }}</textarea>
                                @error('content')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                @if($news->image)
                                    <div class="mb-2">
                                        <img src="{{ asset($news->image) }}" alt="Current Image" class="w-32 h-32 object-cover rounded">
                                    </div>
                                @endif
                            ...
                            <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Update News</button>
