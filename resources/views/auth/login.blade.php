            ...
            <form method="POST" action="{{ route('login') }}" class="lg:w-max md:w-max w-full bg-white p-6 rounded-[5px] shadow-lg shadow-gray-600 flex flex-col items-center w-full">
                ...
                <div class="w-full mt-4 flex flex-col gap-4">
                    <div class="lg:w-[400px] md:w-[350px] w-full flex flex-col gap-1">
                        <label for="login" class="font-semibold text-sm text-black">Email or Mobile Number</label>
                        <x-text-input id="login" class="px-4 py-2.5 text-primaryDark placeholder-primaryDark/60 rounded-[3px] border-[1px] border-primaryDark/60 focus:ring-0 focus:outline-none focus:border-primaryDark hover:border-primaryDark" type="text" name="login" :value="old('login')" required autofocus autocomplete="username" />
