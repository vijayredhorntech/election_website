        ...
        <div class="container lg:max-w-lg  bg-white mx-auto  text-gray-800 rounded-[5px]">
            <div class="bg-white px-4 py-12 flex flex-col items-center text-center rounded-[5px] shadow-lg shadow-gray-800">
                ...
                <form action="{{$formData['url']}}" method="{{$formData['method']}}" class="w-full mt-8 lg:px-8">
                    ...
                    <div class=" w-full flex flex-col items-start">
                        <div class="flex flex-col items-start w-full gap-1">
                            ...
                            <input {{$formData['type']==='validate'?'disabled':''}} value="{{ $userName ?? old('name') }}" type="text" id="name" name="name" class="w-full bg-gray-100 rounded-[3px] border-[1px] border-red-600 px-4 lg:py-3 py-2 focus:outline-none focus:ring-0 focus:border-red-700 placeholder:text-red-500" placeholder="Enter your name.....">
                        ...
                        <div class="flex flex-col items-start w-full gap-1 mt-4">
                            ...
                            <input {{$formData['type']==='validate'?'disabled':''}} value="{{ $email ?? old('email') }}" type="email" id="email" name="email" class="w-full bg-gray-100 rounded-[3px] border-[1px] border-red-600 px-4 lg:py-3 py-2 focus:outline-none focus:ring-0 focus:border-red-700 placeholder:text-red-500" placeholder="Enter your email.....">
                    ...
                    <div class="w-full flex mt-2 {{$formData['type']==='validate'?'hidden':'block'}}">
                        <a href="{{route('login')}}" class="w-full text-center bg-green-500 px-4 py-3 rounded-full text-white font-semibold hover:bg-red-700 transition ease-in duration-200 cursor-pointer"> Alredy a member Sign in <i class="fa fa-angle-right"></i></a>
                    </div>
                </form>
                ...
                <div id="membershipModal" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center z-50">
                    <x-popup type="membership" onclick="toggleMembershipModal()" />
                </div>
